...
and Java. No external API needed (avoids Piston's auth requirement and
Judge0's rate limits entirely).

Requirements on the local machine for each language:
  - Python: no extra install needed (uses the same interpreter running the app)
  - C:      gcc must be installed and on PATH
  - C++:    g++ must be installed and on PATH
  - Java:   JDK (javac + java) must be installed and on PATH

Note: Running arbitrary code locally has the same trust assumptions as running
any script on your own machine. Fine for a personal/student project where you
control what code goes in, but this is not a public-facing sandbox.
"""
